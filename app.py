...
DOC_PATH_RE = re.compile(r"docs/[^\n]*?\.(?:md|txt|pdf)", flags=re.IGNORECASE)
QUOTE_FROM_DOC_RE = re.compile(
    r'"([^"\n]{12,260})"\s*\(from:\s*(docs/[^\n]*?\.(?:md|txt|pdf))',
    flags=re.IGNORECASE,
)
...
def infer_source_doc(file_path: Path, text: str, snippet: str) -> str | None:
    # Preferred signal from extracted source text files generated by builder.py.
    if file_path.name.endswith(".extract.txt"):
        for line in text.splitlines()[:20]:
            if line.lower().startswith("original doc:"):
                candidate = line.split(":", 1)[1].strip()
                if candidate.startswith("docs/"):
                    return candidate

    # Fallback: parse any explicit docs/... citation in snippet or file body.
    for haystack in (snippet, text[:4000]):
        match = DOC_PATH_RE.search(haystack)
        if match:
            return match.group(0).strip()
    return None


def infer_quote_hint(snippet: str) -> str | None:
    match = QUOTE_FROM_DOC_RE.search(snippet)
    if not match:
        return None
...
async def search_docs(payload: SearchDocsRequest):
    ...
    for file_path in sorted(candidate_files):
        ...
        source_doc = infer_source_doc(file_path, text, snippet)
        if source_doc:
            hit["source_doc"] = source_doc
        quote_hint = infer_quote_hint(snippet)
        if quote_hint:
            hit["quote_hint"] = quote_hint
